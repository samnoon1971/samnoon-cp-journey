...
ll solve(ll pos, ll nisi)
{
    ...
    if(nisi == 1)
    {
        if(s[pos] == 'b')
        {
            res = max({res, 1 + solve(pos + 1, 1), 1 + solve(pos + 1, 2)});
        }
        else
        {
            res = max({res, solve(pos + 1, 1), solve(pos + 1, 2)});
        }
    }
    else
    {
        ...
        {
            if(nisi == 0)
                res = max({res, 1 + solve(pos + 1, nisi), 1 + solve(pos + 1, nisi + 1)});
            else
                res = max({res, 1 + solve(pos + 1, nisi)});
        }
        else
        {
            if(nisi == 0)
              res = max({res, solve(pos + 1, nisi), solve(pos + 1, nisi + 1)});
            else
              res = max({res, solve(pos + 1, nisi)});
        }
    }
