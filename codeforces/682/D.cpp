...
const ll N = 1100;
ll dp[N][N][11][2], n, m, k;
string a, b;
ll solve(ll x, ll y, ll rem, ll keep)
{
    if(rem < 0)
    {
        return LONG_MIN;
    ...
    if(dp[x][y][rem][keep] != -1)
    {
        return dp[x][y][rem][keep];
    }
    ll ret = LONG_MIN;
    if(keep == 1)
    {
        if(a[x] == b[y])
        {
            ret = max({ret, 1 + solve(x + 1, y + 1, rem, 1),
                      1 + solve(x + 1, y + 1, rem - 1, 0),
                      solve(x + 1, y, rem - 1, 0),
                      solve(x, y + 1, rem - 1, 0)});
        }
        else
        {
            ret = max(solve(x + 1, y, rem, 0), solve(x, y + 1, rem, 0));
        }
    }
    else
    {
        if(a[x] == b[y])
        {
            ret = max({ret, 1 + solve(x + 1, y + 1, rem - 1, 1),
                      1 + solve(x + 1, y + 1, rem - 1, 0),
                      solve(x + 1, y, rem, 0),
                      solve(x, y + 1, rem, 0)});
        }
