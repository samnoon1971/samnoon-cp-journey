...
bool isHill(ll pos)
{
    return pos - 1 >= 0 && pos + 1 < n && a[pos - 1] < a[pos] && a[pos + 1] < a[pos];
}
bool isValley(ll pos)
{
    return pos - 1 >= 0 && pos + 1 < n && a[pos - 1] > a[pos] && a[pos + 1] > a[pos];
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(ll i=1; i<n-1; i++)
        {
            ll temp = a[i];
            a[i] = a[i - 1];
            ll val = cnt - b[i] - b[i - 1] - b[i + 1] + isHill(i - 1) + isHill(i) + isHill(i + 1) + isValley(i - 1) + isValley(i) + isValley(i + 1);
            mn = min(mn, val);
            a[i] = a[i + 1];
            val = cnt - b[i] - b[i - 1] - b[i + 1] + isHill(i - 1) + isHill(i) + isHill(i + 1) + isValley(i - 1) + isValley(i) + isValley(i + 1);
            mn = min(mn, val);
            a[i] = temp;
        }
        cout << mn << endl;
