...
ll mn[3 * N], mx[3 * N], ar[N];
void build(ll node , ll a , ll b )
{
    if(a > b) return;
    if(a == b)
    {
        mn[node]=mx[node]=ar[a];
        return;
    }
    ll mid = (a+b) >> 1;
    ll L = (node << 1);
    ...
    build(L,a,mid);
    build(R,mid+1,b);
    mn[node]=min(mn[R],mn[L]);
    mx[node]=max(mx[R],mx[L]);
}
pll query(ll node , ll a , ll b , ll i , ll j)
{
    if(a > j || b < i ) return{LONG_MAX,LONG_MIN};
    if(i <= a && j >= b) return {mn[node],mx[node]};
    ll mid = (a+b) >> 1;
    ll L = (node << 1);
    ll R = (node << 1) + 1;
    pll ret1 = query(L,a,mid,i,j);
    pll ret2 = query(R,mid+1,b,i,j);
